define([
    ...
    Status.prototype.render = function(data){
        var $target = $('.source_nav_c');
        var $statusContainer = $('.sourcejs-status');
        var $statusBox = $('.status-box');
        var $statusHtml = $('<div class="source_main_nav_i status-box"><div class="sourcejs-status"></div></div>');
        ...
        _.forOwn(_data, function (value, key) {
            ...
            var label = "default";
            console.log("Key: "+key+" Value.status: "+value.status+" Message: "+msgText);
            $statusContainer.append('<div class="status-message "><p class="status-item status-item-'+ value.status +'">' + key + ':' + value.status + '<span class="status-message-text">'+msgText + '</span></p></div>');
        });

        if ($statusContainer.length === 0) {
            $statusHtml.append($statusContainer);
            $target.append($statusHtml);
        } else {
            $statusHtml.replaceWith($statusHtml);
        }
    };
